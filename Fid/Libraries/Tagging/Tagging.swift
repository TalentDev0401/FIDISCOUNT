...
open class Tagging: UIView {
    ...
    open var currentTaggingText: String? {
        didSet {
            guard currentTaggingText != nil else {return}
//            for item in tagableList {
//                print("name is \(item.username!.lowercased())")
//            }
//            let matchedTagableList = tagableList.filter {
//                $0.username!.lowercased().contains(currentTaggingText.lowercased())
//            }
//            dataSource?.tagging(self, didChangedTagableList: matchedTagableList)
        }
    }
    open var currentTaggingRange: NSRange?
    ...
    public func removeAllTaggedList() {
