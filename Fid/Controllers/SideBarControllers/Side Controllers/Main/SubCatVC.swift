...
class SubCatVC: UIViewController {
    ...
    func selectCatData() {
        switch self.catId {
        case 0:
            self.catArray = FidHelper.shared.getCat1Data()
        case 1:
            self.catArray = FidHelper.shared.getCat2Data()
        case 2:
            self.catArray = FidHelper.shared.getCat3Data()
        case 3:
            self.catArray = FidHelper.shared.getCat4Data()
        case 4:
            self.catArray = FidHelper.shared.getCat5Data()
        case 5:
            self.catArray = FidHelper.shared.getCat6Data()
        case 6:
            self.catArray = FidHelper.shared.getCat7Data()
        case 7:
            self.catArray = FidHelper.shared.getCat8Data()
        case 8:
            self.catArray = FidHelper.shared.getCat9Data()
        default:
            break
        }
    }
    ...
    @IBAction func searchAction(_ sender: Any) {
        print("search Action")
        performSegue(withIdentifier: "search_from_sub", sender: self)
    }
    
    @IBAction func selectCardAction(_ sender: Any) {
        print("select card action")
        performSegue(withIdentifier: "select_card_from_sub", sender: self)
    }
    
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "search_from_sub" {
        ...
        } else if segue.identifier == "select_card_from_sub" {
            let destination = segue.destination as! SelectCardVC
            destination.delegate = self
        } else if segue.identifier == "category" {
            let destination = segue.destination as! CategoryVC
...
extension SubCatVC: SelectCardVCProtocol {
...
extension SubCatVC: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        cell.image.image = UIImage(named: self.catArray[indexPath.row].key!)
        cell.title.text = self.catArray[indexPath.row].value
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        self.selectedCardTxt = self.catArray[indexPath.row].value
    ...
    func collectionView(_ collectionView: UICollectionView, layout collectionViewLayout: UICollectionViewLayout, sizeForItemAt indexPath: IndexPath) -> CGSize {
        ...
        if self.catId == 0 || self.catId == 3 || self.catId == 4 || self.catId == 5 || self.catId == 7 {
            itemSize = (collectionView.frame.width - (collectionView.contentInset.left + collectionView.contentInset.right + 20)) / 2
            return CGSize(width: itemSize, height: itemSize)
        } else {
            itemSize = (collectionView.frame.width - (collectionView.contentInset.left + collectionView.contentInset.right + 30)) / 3
